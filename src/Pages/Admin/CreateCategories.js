  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "https://aapla-market-backend.vercel.app/api/v1/category/create-category",
        categoryData,
        {
          headers: {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `https://aapla-market-backend.vercel.app/api/v1/category/update-category/${selected._id}`,
        { name: updatedename }
      );
      if (data.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(
        `https://aapla-market-backend.vercel.app/api/v1/category/delete-category/${pId}`
      );
      if (data.success) {
